...
Line 30 is an example of a compound statement, with two statements separated by a colon. The first statement is an assignment using the addition operator with the value stored in `A` and the numeric constant 5 as the expressions. This will have the effect of assigning the resulting value 15 to the variable `C`. The second statement on this line is a print statement, another very common statement in most programs. This will evaluate the expression, returning the numerical value 15, convert it to a string, and then print that value to the user's console.

Line 40 shows an example of *overloading*, where a keyword may mean different things depending on the context. The expression in the print statement has the `+` operator but is given **string expressions** instead of numeric. In this case, the `+` does not represent numeric addition, but *string concatenation*, which places the right-hand string at the end of the left-hand string and produces a new, longer string. The result of this print statement is `Hello, World!` being printed to the console.

Line 50 is an example of a logical comparison, which is indicated by the `IF` statement keyword. This is followed by a **logical expression** (*lexp*) using the larger-than operator, which calculates the results of expressions on both sides of the operator, compares them, and then, depending on the outcome of the comparison, optionally performs the statement following the `THEN`. As `C` will hold the value 15 at this point, and 15 is larger than 10, the statement after `THEN` *will* be performed and print `C is > 10` to the console.
...
One important oddity of BASIC is that it does not require delimiters between keywords and expressions. This contrasts to most modern languages, which demand some sort of indication that a keyword has ended, normally a space, or other punctuation like the end of line. At the time BASIC was written, memory was so limited that leaving out these characters added up to a worthwhile savings, despite its effects on the readability of the resulting code. So, the code above could be written:

    10LETA=10
    20B$="Hello"
    30C=A+5:PRINTC
    40PRINTB$+", World!"
    50IFC>10THENPRINT"C is > 10"
    60END

RetroBASIC can process many programs that are found in this "compressed" format, but the technology it is based on, flex and bison, will have problems in certain cases. Please [file a bug report on GitHub](https://github.com/maurymarkowitz/RetroBASIC/issues) if you find an example of such code that will not parse correctly. The code in the following documentation always includes spaces for clarity.
...
Some dialects had a second number format, *integer*, or what would normally be called a "counting number" or "whole number". Later versions of Microsoft BASIC added support for integers, so this feature is widely available in home computer dialects. MS used 16-bit values for integers, potentially saving considerable amounts of memory in programs with large numbers of numeric constants. To indicate that a variable should store the value in integer format, the percent sign, `%`, was added to the end of the variable name. So, `I` would store a value in the default format, normally a 40-bit floating point value, while `I%` would store the value as a 16-bit integer. Although integer support was common on later machines, it was not backwardly compatible with earlier dialects, so it was rarely used in programs. 
...
In MS dialects, the existing floating point code is used to perform all calculations, even those on integer values. This allows them to support integer values without having to write a separate math library, which would be difficult given the very limited memory of the early machines where every byte counted. If the result of an expression is being stored into an integer variable, the `INT` function is applied to the result. That means `A#=7/3` will result in 2 being stored in `A#`. However, converting back to integers means integer code actually runs (slightly) slower than using floating point. In later dialects, like BBC BASIC, an separate all-integer math library is used for this purpose. This offers higher performance, sometimes significantly.

RetroBASIC uses the 64-bit IEEE double format to store all numbers, even integers. RetroBASIC also carries out all calculations using doubles, as performance is no longer an issue. The use of doubles is hidden from the user and has no effect on the operation of programs, `PRINT`ing one will result in the correct output format no matter the internal representation. Like MS, it applies `INT` to the resulting values if the result is being stored in an integer variable.

BASIC inputs and outputs numbers in decimal format, using the same formatting rules as the C language "g" format. This means that large numbers and small fractions are output in exponent format, regardless of how they are input. For instance, if the program asks the user to type in a number and they type fifteen ones, when output that value will be rendered something like ` 1.11111E14`. For those dialects that supported doubles, numeric constants could be set to doubles by changing the E to a D. This changed only the storage format, they still printed the same way.

RetroBASIC has the additional feature that numeric constants can be entered in hexadecimal, octal or binary format. Hexadecimal values are indicated by placing the prefix `0x`, `0h` or `&` in front of the value, for instance, `&1F3A` will be converted internally to the decimal value 7994. Octal is indicated by `0o`, like `0o3725`, and binary with `0b`, like `0b10110011`. These values can be used in any numeric expression in place of a decimal constant. Like any numeric value, they are stored internally as IEEE doubles.

Because the input and output are in decimal, but the internal format is in binary, there are numbers that one can type into the source code cannot be exactly represented internally. This is similar to the way one cannot exactly represent 1/3 in decimal, which we indicate using an ellipsis, writing 0.333... . Generally, when moving values between number formats there will always be some values that cannot be exactly represented.

As a result of these differences between binary and decimal storage, BASIC programs are subject to "round-off error" that can produce odd results, especially when dealing with fractional values. It is widely suggested that you do not rely on a variable containing a specific value when testing it; `IF X=5 THEN A=1` will fail if the value of X rounded up to 5.000101 during a previous calculation. Solutions to this sort of problem lead to messy and confusing code. RetroBASIC's double format is not immune to round-off error, although it is *greatly* reduced in frequency by the higher-accuracy numbers being used internally and additional features of modern floating point hardware. As such, this sort of error is unlikely to be seen and this sort of confusing code can be avoided in most cases.

To avoid round-off errors, some dialects used a different format known as binary coded decimal, or BCD, which stores decimal numbers directly. This means that any number one can represent in the source code can be exactly represented in memory. This does not eliminate round-off in calculations, one can still perform `A=1/3` and produce a rounded-off value, but the cases where this occurs are more obvious to the user. The downside to BCD is that the numbers take up slightly more memory and are generally slower to perform math on. This was a small price in some settings, and those machines intended for business roles like the Wang 2200 used BCD, as do most handheld calculators. Although not unheard of in general-purpose dialects, the only widespread versions to do so in the home computer era were Atari BASIC and TI-99 BASIC, as well as the later MSX dialect.
...
String constants are normally delimited with double quotes, like `"Hello, World!"`, although some dialects use single-quotes, `'Hello, World!'`, and a very few allow both formats. Dialects vary widely in the way that they include quotes *inside* string constants; the modern solution of placing two double quotes beside each other inside a string, like `"Hello, ""World""!"`, is a much later invention.

Another curiosity of most BASICs, also seen in the FOCAL language, is that the closing quote can be left off if it is the last character on the line, although this is *rarely* (if ever?) seen in BASIC programs. This means that `10 A$="Hello, World!` is functionally the same as `10 A$="Hello, World!"`, but saving one byte. On early machines, this could be useful.
...
The 1968 version of Dartmouth BASIC introduced string variables. To indicate it held a string, the dollar sign, \$, was added to the end of the name. Because the \$ is part of the name, and most dialects only allow for two characters in the name, it is fairly common to have the limitation that string variables can only have single-character names.
...
RetroBASIC parses keywords like `PRINT` in a case-insensitive manner, so one can use `PRINT`, `print` or even  `PRinT`, and it will be understood.

This is **not** the case for variable names. This means `I` and `i` are two different variables, as are `VAR` and `Var`. This *appears* to be the case for most interpreters that allowed lower-case typing in source code, but more examples would be nice to have.

#### Notes:

Most BASIC interpreters store the values of the variables in a list. The first two locations in each entry hold the variable's name. When the program refers to the variable in code, the interpreter searches through the table from top to bottom until it finds an entry with that name. If it does not find a match, it creates a new entry at the bottom of the table and inserts the name there.
...
Not all dialects work this way. Compilers, like Dartmouth, hold links directly to each variable's storage in memory and thus do not have to search the table at runtime. A number of interpreters use the same strategy, Atari BASIC being a notable example. From a performance standpoint, in these dialects it is as if every variable is at the top of a one-entry list. While manual caching will still run, it offers no performance advantage.

<!-- TOC --><a name="arrays"></a>
### Arrays

Most BASIC dialects include the concept of an *array*. An array is a group of related values all referred to by a single variable name. Each value is assigned a *slot* in the array, also known as an *element* or or *entry*. Most arrays have to be defined before use using the `DIM`ension statement, although there is an important exception (see below). Later in the program, the separate values can be referred to using numbers in parentheses placed after the variable name. These numbers are known as *indexes* or *subscripts*. For example:

    10 DIM A(10)
    20 A(1)=5
    30 A(2)=10

This program sets aside eleven locations in memory, or slots, that can hold numeric values. It then assigns the values 5 to slot 1 and 10 to slot 2. The rest have not been set, and those slots will contain zeros.

The index numbers can also be held in variables, or any other expression. For instance, the following code is logically identical to the example above:
...
The exact handling of arrays depends on the dialect. Some allow a single dimension, while most allow two, and a small number allow three or more. RetroBASIC allows any number of dimensions and should properly run code from any dialect. In those dialects that allow two or more dimensions, single-dimension arrays, like the example above, are sometimes referred to as *vectors*, or in a few occasions, *lists*, while those with two dimensions are sometimes known as *matrixes*, or alternately *tables*. When talking about arrays, it is also common to refer to non-array numerical values as *scalars*.

Two dimensional arrays are fairly common, for instance:
...
There is another oddity to consider. Dartmouth sets aside a 0...10 block of memory for every numeric variable, even if it is not `DIM`med. In the example program above, A is defined to have 11 slots, and then it assigns values to the slots. This program will also work if line 10 is left out, because A will always have the default 11 slots. This is a *very* odd feature and entirely non-obvious to the point that it is rarely implemented in non-MS dialects. DEC BASIC's did copy this feature, and that did move to MS, although to this day it remains pretty much unknown and unmentioned. RetroBASIC does implement this "feature" in case there are programs that use it. To date, only one program found in the wild has made use of this feature, in every other example the arrays are `DIM`med.
...
To summarize, consider this program using a one-dimensional array following the MS conventions:
...
Some dialects allow a parameter to be passed to define how much memory to reserve for string storage. Examples include BASIC-80 and TRS-80 Level II. This is not supported in RetroBASIC, which has (effectively) unlimited memory.
...
`REM`, short for "remark", is used to insert comment text into the program code. Any text on the line after the `REM` is ignored, and execution immediately moves to the next line. RetroBASIC supports the alternative versions, `'` and `!`. The former was first introduced in Dartmouth v4 and is widely used in MS-derived BASICs. The latter, taken from Fortran, was first found in Tymshare SUPER BASIC, and is seen in a few other dialects. The short forms only work if they are the first non-whitespace character in a statement, the `!` serves other purposes in some dialects but will always be found following a statement keyword in those cases.
...
In those dialects descending from HP, string arrays are actually arrays of single characters, and not a separate "string object". In Atari BASIC, for instance, `DIM A$(100)` is not making an array of 101 strings but a single string with up to 100 characters. In most such dialects, there is no way to make an array of strings, which can make some programs difficult to construct. Later dialects, like ANSI Full BASIC, use an alternate syntax that allows for real string arrays as well as the "slicing" syntax of HP's model.

For wider compatibility, RetroBASIC normally assumes a `DIM` on a string is defining an array of strings. This can be changed by turning on slicing with the `--slicing` command line switch, in which case `DIM`s follow the HP pattern.
...
Enterprise IS-BASIC allows the `TO` keyword to define the upper and lower limits of the array. For instance, `DIM A(10 TO 20)` defines an array with 11 entries, from 10 to 20 instead of 0 to 10. This is not supported in RetroBASIC, and the utility of this concept is questionable.
...
Univac 1100 BASIC allowed you to specify the target as an offset rather than an absolute number. For instance, `GO TO *5` would branch to the line five down from the current location, while `GO TO *-3` would branch backward 3 lines. The idea was to make the targets independent of the numbers, so the line numbers could be changed with no requirement to update the branches. The GRASS language had a similar feature, `SKIP 5`. Neither is supported in RetroBASIC.

Cyber BASIC from the CDC mainframe line included a `JUMP` statement that was simply a `GOTO` with a numeric expression instead of a numeric constant. They added this specifically to handle the need to branch out of error handling (see section below) as one could `JUMP ESL` to return to the line that raised an error, or `JUMP NXL(ESL(0))` (NXL returns the next valid line number from a given line number) to continue past the error. The manual explicitly states one should never use this in place of a `GOTO`, but fails to explain why. Many other dialects did support this in their `GOTO` statements, as does RetroBASIC.
...
The name "GOSUB" is a contraction of "go to subroutine". Subroutines are small sub-programs that perform a specific task. For instance, one might write a subroutine to calculate the circumference of a circle with a given radius and then call that code repeatedly with different input values from different locations in the code. This allows the calculation code to be isolated, often placed near the bottom of the program to separate it visually, and then called from different parts of the rest of the program. This way the code does not have to be duplicated in all of the locations that use it.

`GOSUB` statements are unconditional branches like a `GOTO`, but differ in that they record the position in the code where they were called from. They do this by writing down the location of the `GOSUB` in a special area in memory known as the **runtime stack**, or simply the **stack**. They then branch to the target line number as would a `GOTO` and continue as normal until they encounter a `RETURN` statement. At that time, they read the entry on the stack they added earlier, remove it from the stack, and branch to the location they stored and continue at the statement after the original `GOSUB`.

Subroutines are normally written at the end of the program code to separate them out. As the lines of code making up subroutines are normal statements, programs might accidentally run them when they reach the end of the main program code. If this occurs, the `RETURN` at the end of the subroutine will be called without a `GOSUB` having placed information on the stack, and an error will occur when `RETURN` attempts to retrieve the return location. To prevent this, programs would sometimes place their `END` statement immediately before the subroutines, although this was sometimes considered bad form. Others would place the `END` at the end as expected, and then place a `STOP` above the subroutines. Still others would place the `END` at a high line number like 50000 and then `GOTO 50000` at the end of the main code.

Most interpreted dialects of BASIC, including most microcomputer varieties, will search for a branch target by starting at the top of the program and then looking through the entire source code for the matching line. Normally, subroutines are placed at the end of the program using large line numbers to make them more obvious. For instance, the subroutines might start at line 10000. However, this also maximizes the amount of time it takes to find the subroutine at runtime and slows performance. Programs looking to improve performance would sometimes place their subroutine code near the top of the program and then have a `GOTO` to jump past them when the program started. This led to the code being difficult to read, but running much faster if those routines were widely used. This sort of trick is not required in RetroBASIC, which can find any line in a single operation.

RetroBASIC supports the seldom-used option introduced in MSX BASIC and BASIC-A that allows a line number to follow the `RETURN` statement. This is exactly equivalent to calling `POP` or `EXIT` followed by a `GOTO` to that line.
...
BASIC75 has a `USING` statement that works like a one-line `GOSUB` with no `RETURN`. For instance, `USING 10000` will jump to the code at line 10000, perform it, and then return. A second form of the same command worked like an `ON`, `USING X OF...`. This same behaviour can be accomplished using `GOSUB` or `ON X GOSUB...` and putting a `RETURN` at the end of the line, so it is not clear why they added new syntax for this. It may be they were trying to duplicate the similar `Do` feature from the JOSS and FOCAL languages.
...
#### Examples:
    ...
    400 PRINT "A IS 1":GOTO 700
    500 PRINT "A IS 2":GOTO 700
    600 PRINT "A IS 3"
    700 END

This program will print a `A IS 1`, `2` or `3` depending on the value in the numeric variable `A`. Line 10 assigns the value 2 to A, so the program will print `A IS 2`. If line 10 is modified to set `A` to a value larger than 3, 10 for instance, the `ON` will do nothing and the program will continue on line 400.
...
`LABEL` is an assignment statement, like `LET`, that sets the value of the *nvar* to the current line number. This is intended to be used with branches that wish to use a name instead of a numeric constant.

Some dialects of BASIC, like Atari and Tiny, allow any numeric expression to be used as a line number target, which means one could simply set a variable to the target line number and use that, like `A=100... GOTO A`. This works in RetroBASIC as well. However, if the code is modified and the target changes line numbers, the variable will no longer point to the right line of code.

`LABEL` addresses this problem by automatically setting the value of the variable to the current line number. This way, if the code is modified, the variable will automatically be assigned the new number when the program next runs.

A number of dialects included a `LABEL`-like feature, but there was no standard syntax for this concept. For example, Acorn Atom BASIC had labels that were implemented by typing a single lower-case letter after the line number on the line to be labeled, and then you could `GOTO s`.
...
### `FOR` *avar*`=`*aexpr1* `TO` *aexpr2* [`STEP` *aexpr3*] [*statmnt*...] and `NEXT` [*avar*,[*avar*,...]]

The FOR/NEXT loop performs a sequence of statements a set number of times. The statements are those between the `FOR` and its corresponding `NEXT`, which is known as the *body* of the loop. The same behavior can be accomplished using IF statements, but FOR/NEXT loops are optimized to make them run much faster.
...
Using FOR/NEXT for this sort of task, instead of IF, more clearly indicates the intentions of the programmer.
...
This loop demonstrates a key concept in the FOR/NEXT implementation:
...
The DEC EduSystem BASIC uses `SLEEP` instead of `PAUSE`, and the time is specified in seconds, not jiffies. The manual is not entirely clear on what happens during a `SLEEP`; BASIC-PLUS states that it "is used to dismiss the currently running program for the number of seconds... At the end of this period the program is again runnable." This appears to mean that the program does not pause and wait, but gives up its time to other programs on the system.

#### Notes:

The definition of a jiffy varies widely across platforms, so a `PAUSE` statement found in existing code may not pause for the correct amount of time. See details in `TIME`. In contrast, `SLEEP` is always in seconds.

#### Variations:

DG BASIC and BASIC75 uses `SUSPEND` using seconds, and allows other programs to run like the DEC `SLEEP` variation. This is not currently supported.
...
`SLEEP` was added in 2.0.0.

<!-- TOC --><a name="call-aexpaexp-exec-system-and-sys"></a>
### {`CALL`|`EXEC`|`SYSTEM`|`SYS`} *aexp*{,*aexp*...} 

`CALL` is one of the two common ways that BASIC supported the use of machine language code in a BASIC program. This was often used to access operating system routines and sound and graphics commands. The required *aexp* represents a memory address where execution should begin. The machine language code is expected to call that platform's equivalent of a `RETURN` at the end of the code, picking up at the next statement in the BASIC program.

When these `CALL`s are encountered in RetroBASIC, they perform no action.
...
Some dialects allowed parameters to be passed in after the address. Amstrad CPC allowed a single parameter, BBC BASIC allowed a list of them.

Later IBM PC dialects used `SYSTEM` as an alias for `BYE`, exiting to the DOS shell.

Dartmouth BASIC used `SYSTEM` to load a language, either BASIC or ALGOL. It was not part of the BASIC language itself, but the underlying command shell.
...
`OPTION BASE` was introduced in Dartmouth BASIC version 4. It allows you to select whether arrays start at index 0, or 1. Earlier versions used base 1, and when base 0 was introduced in version 2 that meant that all arrays ended up with additional unused elements. For those programs that used a base index of 1, which is many of them, using `OPTION BASE 1` before using `DIM` statements would eliminate this wasted memory.
...
`INPUT` can accept multiple values in a single statement, using a comma-separated list of variables. The user may enter any value on its own and press <return>, or they can enter multiple values on a single line by separating the values with commas. If the variable is a string, a comma can be typed within the string by surrounding the entire string with quotes, in the same fashion as a `DATA` statement. When there are multiple variables in the `INPUT`, if the user types fewer than that number on any one line, the system will print a `??` to indicate more data is needed.

The handling of missing or empty inputs, which occurs when the user hits <return> without typing a value, varies across implementations. Some dialects, like early Commodore BASIC on the PET, stop execution of the program and return to the READY prompt without reporting an error. Atari BASIC will exit and print error 8, "INPUT or READ type mismatch". Others, like Commodore BASIC V2 on the C64, continue execution leaving the variable's previous values unchanged. RetroBASIC follows this last pattern, as it appears to be the most common implementation and has no real downsides.
...
Although the question-mark prompt is *almost* universal, Tiny BASIC uses a colon, `:` while PRIME and TSC Extended BASIC uses a bang, `!`.

Some dialects allow the question-mark (etc.) to be suppressed, but there is no standard for this feature. The most common variation is the one used in QBASIC and GW-BASIC, which prints the question mark if the prompt separator is a comma, but not if it is a semi-colon. Unfortunately, many dialects demand one or the other separator so this cannot be cleanly supported. RetroBASIC uses comma and semicolon to space out the prompts like a `PRINT` statement.

IBM 5100 BASIC requires the user to type single quotes around string entries made by the user, but this is not needed in most dialects, including RetroBASIC.
...
In contrast, using INPUT:

    10 INPUT "Type anything:",A$
    20 PRINT "You typed: ";A$
...
A curiosity of the BASIC language is that the question-mark may be used as an alias for the `PRINT` statement. This was intended to allow users to type in "questions" at the command line, for instance, `? 5+5`, meaning "what is 5 plus 5?" The interpreter would internally expand the question-mark to `PRINT`, perform the expression, and print the result, `10`. As the question-mark is expanded on entry, in most dialects if you enter the statement using the question-mark, it will expand it internally and `PRINT` will appear when `LIST`ed. Some programs, including *Super Star Trek*, used this as a trick to enter lines that might otherwise be a few characters too long to be entered on some machines.
...
It is not easily visible in that example, but there is an additional leading space in front of the value 9. This is part of the formatting rules for numeric values; it leaves room to print a minus sign for negative values, which is left blank if the number is positive. This way, if a series of numbers is printed that may be negative or positive, they will properly line up vertically on the console.
...
MAI Basic Four does not use the `USING` keyword, instead, any `PRINT` statement can add a format string to any single parameter by placing a colon after it, like `PRINT A:#,###.##`. The colon can also be used as the first statement on any line to produce an `IMAGE`-like line. This is not currently supported in RetroBASIC, as it overlaps too strongly with the normal use of colons as statement separators.
...
Writes a single byte from *var* to the screen. If *var* is a numeric variable it prints the ASCII character with that value, if it is a string variable, it prints the first character of that string. Its counterpart for input is `GET`.
...
Early versions of Dartmouth BASIC lacked the ability to work with data files, which made it difficult to store data for use in a program. While one could implement this using multiple assignment statements, this made the code lengthy and difficult to understand. To solve this problem, the `DATA` statement was introduced, which allows constant values to be stored in a compact format. The `READ` statement retrieves these values, and `RESTORE` resets the `READ` to the first `DATA`. The system simulates reading data from a file that can only read forward and then be re-wound to the start.
...
As the separator is the comma, strings that contains commas have to be quoted:

    70 DATA "this is a string, and it has commas",but this one does not
...
`RESTORE` is used to reset the data pointer to the start of the list of `DATA` statements, or to the first data element on a particular line indicated by the optional line number or numeric expression.
...
The length of the string is stored in the zero slot, so if the string length changes this slot has to be changed as well. For instance, we can modify the code above to add "world" to the end of the string:
    ...
    90 X(0)=10
    100 CHANGE X to X$
    110 PRINT"The string has been modified to "X$
...
RetroBASIC supports the standard set of arithmetic operators: `+` for addition, `-` for subtraction, `*` for multiplication, `/` for division, and `^` for exponentiation. It also supports the alternative `**` for exponentiation, which is seen in some older dialects from machines that did not have a ^ character on their keyboards.
...
RetroBASIC supports the standard set of logical operators: `=`, `>`, `<`, `<=`, `>=` and `<>` for not-equals. It also supports the HP-style `#` which is the equivalent to `<>`, as well as the relatively rare `><`, `=>` and `=<` variations seen in some dialects like MAI Basic Four.

RetroBASIC supports the basic logical operators `NOT`, `AND` and `OR`, as well as the less-seen but useful `XOR`.

RetroBASIC also supports two less well-known logical operators, `EQV` and `IMP`. `EQV`, for "equivalent", returns true if both of the parameters are the same, that is, both are true or both are false. This contrasts with `AND`, which would return true only if both are true. `IMP`, for "implies", returns true only if the first parameter is true and the second false, otherwise it returns false.
...
According to Lien, some computers accept `A` as a short form, but fails to specify which ones. It is unlikely this is true, as it would be difficult to know if something like `C=A(B)` is referring to the `ABS` function or a an array variable named `A`. It is more likely he is referring to the way some systems allow you to enter abbreviations, so `A` expands to `ABS` in a similar to the way `?` expands to `PRINT`. RetroBASIC does not support this short form.
...
Performs an integer division on the two parameters. For instance, `DIV(7,2)` returns 3, the integer result of the value 3.5.
...
BASICs originating in the UK, including BBC and Sinclair, among others like the Sharp MX, use `LN`. On the UK machines, `LOG` returns the common logarithm, which most dialects support using `CLOG`. This may lead to some problems when running programs from these platforms. This is supported in RetroBASIC despite it meaning that variables called `LN` may have problems.
...
Tymshare SUPER BASIC and Wang BASIC allow any number of expressions in the parameter list in the functional style, this is not currently supported.

According to Lien, some dialects allowed a single variable to be used if it was an array, returning the largest value in the array. Which dialect this might be is not mentioned. This is not currently supported.
...
The ability to have more than two parameters was added in 2.2.0.
String parameters were added in 2.2.0.
...
Returns the modulus, or remainder, of the division of *aexp1*/*aexp2*. `MOD(7,3)` returns 1 while `MOD(8,2)` returns zero. The function performs the equivalent of `INT(X - Y*INT(X/Y))`, so `MOD(7,3)` performs INT(7 - 3\*INT(7/3)) = INT(7 - 3\*2) = INT(7 - 6) = 1. This functionality is also available as an operator, where this same call would be written `7 MOD 3`.
...
Returns a random positive number between 0 inclusive and 1 exclusive, so 0 to 0.999... Some dialects require some form of expression in the parentheses even if they do not use it, others do not require this and you can leave the parameter empty, and Dartmouth and its many clones do not even need the parens. RetroBASIC works with any of these styles. In many dialects, if a variable or expression is passed in the parentheses it is ignored and has no effect on the numbers returned, but there are numerous important exceptions. The variations are so (*ahem*) random, that Lien's *The BASIC Handbook* gives up and tells you to consult your manual.

Most random number generators are based on a mathematical formula that takes the last random number as its input. This means that each call to the function is guaranteed to return a new value in a sequence. Systems vary widely on how this is actually implemented. In some, the initial number, or *seed* is always the same and thus the *sequence* of numbers generated will always be the same. These variations generally offer some method to change the initial value so that different sequences can be generated. Earlier dialects mostly use the `RANDOMIZE` statement for this, but later ones generally modified the `RND` itself for this task.

Some home computer dialects, like Atari BASIC, used an internal hardware timer to produce random values. This was much faster than using a complex function, and also guaranteed that the values were always randomized even without a seed. Some blended the two options, allowing you to set a seed if you wanted to create a known sequence, but otherwise returning random values from hardware.

`RND` is very common in BASIC programs, especially in games. In most cases, the program actually desires an integer value, choosing a value between 1 and X. To produce such a value, code to the effect of `INT(RND(0)*X+0.5)` can be found in many programs. This bit of code is so common that some programs use a user-defined function to make references to this sequence of operations shorter; an example is *Super Star Trek*, which defines a function called `FNR` near the top of the program and then calls it from many locations, making the code more compact.
...
It is generally the case that any dialect that has the `RANDOMIZE` statement will treat the *aexp* in `RND` as a dummy. RetroBASIC is an exception, as it supports setting the seed in `RANDOMIZE` or `RND`. Those dialects that do use `RANDOMIZE` generally have no parameter or use a standard one like 0 or 1, and will thus run fine without modification.
...
Sinclair QL BASIC can generate numbers in any given range using `TO`, like, `RND(10 TO 50)`.

Commodore BASIC uses `RND(0)` to produce a "randomized random value", which is accomplished by mixing the last value with the value from an internal timer. As the timer functions vary across their computer models, the outcome of this function also varies. On most machines, the timer is always running and `RND(0)` operates as if it has been randomly seeded using `RANDOMIZE`. On the Commodore 64, the timer does not start automatically and `RND(0)` will always return the same value. Note that this is different than dialects like Dartmouth; those will return different values for `RND(0)` every time it is called, although the *sequence* of values will always be the same (unless `RANDOMIZE`d). On the C64, the *value* is the same not only every time the program is run, but also every time the function is called. To address this, users were told to add `RND(-TI)` near the start of their programs, using the pseudo-variable `TI`, which contained the number of ticks since the last reset, as the seed. This is not needed in RetroBASIC, but will work property when encountered.
...
Early Dartmouth dialects returned 1 for all positive numbers, including 0. Almost all later dialects, including later Dartmouth versions, return 0 in this case. RetroBASIC only supports the later version with 0.
...
BASIC interpreters display and input numbers in decimal format. In many programs, especially on home computers, it is more convenient to type certain values in other formats, like hexadecimal or binary. RetroBASIC includes a number of functions to convert to and from these formats.

RetroBASIC also allows binary, octal, and hexadecimal values to be typed in directly as constants, using the `0b`, `0o` and `0h` prefixes, respectively, as well as `&` for hexadecimal, which was found in some dialects like BBC. For instance, `0b10101010` represents the decimal value 170 entered as a binary constant.

These are particularly useful when entering bit-oriented values like masks for fonts and sprites, as well as referring to memory locations which were often specified in hexidecimal in machine documentation.
...
MAXBASIC allowed a second parameter, a length, which returned a series of values, up to that many characters in the string. This not supported in RetroBASIC, but the functionality is similar to the `CHANGE` statement in Dartmouth BASIC, which is supported.
...
Acorn Atom BASIC used `CH`, for "character". This is not supported.

Enterprise IS-BASIC uses `ORD` for no obvious reason. This is not supported.
...
The `INKEY` function returns the ASCII value of the key currently pressed on the keyboard. If no key is pressed when it is called, it returns 0. In either case, it immediately continues without waiting for user input, unlike `INPUT`.
...
BBC BASIC will calculate *dexp* and if it is non-zero, wait for that many 1/100ths of a second for input, instead of continuing immediately. This is not currently supported in RetroBASIC.
...
A number of BASICs, including AppleSoft, Apple Business, Atari, BBC, etc., used the `GET` statement for similar functionality. Some of these waited for a keystroke before continuing, and thus did not serve exactly the same purpose. On these platforms, `INKEY$` was often implemented using `USR` or `PEEK`. On AppleSoft, one could optionally use an *aexp*, with some caveats. Apple versions also did not echo the typed character.
...
Univac 1100 BASIC adds the associated `CNT` function, which returns the number of times the pattern is found in the string. This was intended for constructing loops as one could loop from `1 TO CNT("Hello", A$)`. This same functionality can be constructed using an `IF` statement that checks if the returned value is 0, but in that case it can take several lines of code to produce the same result. `CNT` is not currently supported.
...
#### Examples:
    ...
    30 PRINT "The string from 7 for 3 is "MID$(A$,7,3)
...
Apple Business BASIC has the similar-looking `SUB$`, but this is short for "substitute", not "substring". This performs replacement of one portion of a string with another, which is useful in a system with MID/LEFT/RIGHT, where this sort of operation can be complex to implement. For instance, `SUB$("Hello!", "XX", 2)` produces HXXlo! RetroBASIC does not support this variation, but it can be accomplished using string slicing.

Wang BASIC from the 2200 uses `STR`, with no `$`, as the equivalent of `MID$`. This is also used in the dialect used in the game Survival from the January 1982 issue of Creative Computing, which appears to be an S-100 port of Wang BASIC. This variation is not (currently) supported in RetroBASIC.
...
`SPC` returns a string containing the number of space characters in *aexp*. `SPA` is the alternate spelling for the same function found in HP, while `SPACE$` is found in other dialects. As is the case for `TAB`, in most dialects this is a special-purpose function that can only be called inside `PRINT` statements. In RetroBASIC it can be used as a normal string function in any string manipulation code.
...
MAI Basic Four allows formats to be specified in the `STR$`, for instance, `A$=STR$(1000":#,###")` will result in A$ containing `1,000`. This is not currently supported in RetroBASIC due to the use of the colon overlapping its normal use as a statement separator - Basic Four uses semicolon for that instead.

Wang BASIC from the 2200 uses `STR`, with no `$`, as the equivalent of `MID$`.

<!-- TOC --><a name="valsexp"></a>
### {`VAL`|`NUM`}(*sexp*)

This function returns a number of the same value as the number stored as a string, performing the opposite operation of the `STR$` function. For instance, if *sexp* evaluates to "25", this function will return the numerical value 25. `VAL` is generally more common than `STR$` as it is often used to convert user inputs into numbers for further processing.
...
RetroBASIC is designed to run any program that does not use platform-specific features. As string arrays are relatively commonly, it has to support them. This leads to it having to decide whether a reference in code like `A$(5)` is a slice or an array access. In the case that the syntax is clear, like Sinclair or Full, there is no ambiguity and one can use string arrays and string slicing in the same program using syntax like `A$(5)(5 TO 10)`, meaning the substring from the 5th to 10th character of the string stored in slot five of the array `A$`.

To work with the dialects that do not have unique syntax, like Dartmouth or Atari BASIC, RetroBASIC offers two solutions. One is to use HP's 'square brackets in place of parentheses, as `A$[5]` is always a slicing. Alternately, it can be turned globally on using the `--slicing=1` command line switch, in which case all "array like" parameters applied to strings are considered slices.
...
This may seem like a small difference, but it had a significant effect in terms of memory use. The string functions return new strings that have to be combined together in the assignment, so the A$ in line 20 is an entirely new string. In almost all implementations, the original value of A$ is left in memory as well. Programs that perform string manipulation using these functions often run out of memory performing even simple tasks. In contrast, using slicing means the assignment is editing the original string in memory, so no additional memory is used. This can offer significant real-world savings in both memory and performance.
...
Every time through the loop, a new version of `B$` is created, one character longer than the last. If this uses up available memory, MS BASIC runs "garbage collection" code to reclaim unused memory, which is slow, buggy, and from the user's perspective, randomly stops the program. In contrast:
...
This leads to one of the few cases where RetroBASIC cannot automatically determine the dialect from the input. If the original program is written in HP BASIC *and* the author chose to use parentheses for slicing, it is up to the RetroBASIC user to address this with the `--slicing=1` switch.

#### Variations:

String slicing was common in early microcomputer BASICs, which were often written by programmers that were familiar with HP BASIC. The best example is Apple's Integer BASIC, which Woz wrote based on a manual he read while he worked at HP. Many other early dialects also copied this style, Atari and North Star BASIC for instance.
...
Data General BASIC used an alternate syntax for this, `DIM A*7` means a string with seven characters maximum, which allows `DIM A$(25)*7` for an array of 25 7-character strings. DG BASIC also used the colon within the slice, like `A$(2:5)`

Another dialect that supported arrays and slicing was ANSI Full BASIC, which used a separate colon-separated parameter. `A$(1)(1:1)` is the first character in the first slot of the A$ array.
...
Those dialects that allow the program to reserve space in memory for strings, typically using `CLEAR`, generally also allow that amount to be checked by passing in a string variable name. In this case, `FRE()` will return the total amount of memory available, or the amount of memory available for program code, while `FRE(A$)` will return the amount of memory in the string area (`A$` in this example can be any string variable name). In early dialects, the amount of string memory was often as little as 50 to 200 bytes.

The TRS-80 II and Genie/Color Genie uses `MEM` for this function, while `FRE` always returns string space. The Dragon 32 uses `SIZE`.
...
The main purpose of this function is to return the number of items typed into the last `INPUT` or `MAT INPUT` statement. This was initially introduced in dialects that had matrix support, as it was common for users to enter a blank value in order to stop the `INPUT` statement in the case of large matrices - they may only want to type in five values for a 10 by 10 matrix for instance. The program could then use `NUM` to test whether the entire matrix had been filled out, and then, for instance, fill out the slots for any non-entered values with zero or some other value.

In RetroBASIC, the functionality of `NUM` has been expanded to also work with conventional `INPUT` statements. Early dialects of BASIC generally stopped execution if the user entered an empty value, typically by pressing <return>. Commodore BASIC on the PET is an example. Later versions mostly allowed empty inputs and simply continued execution from that point. The C64 is an example of this style. In this latter case, `NUM` is just as useful in normal `INPUT` statements as it is in `MAT`.
...
Returns the contents of a specified memory address location *aexp*. The address specified must be an integer or an arithmetic expression that evaluates to an integer between 0 and (typically) 65535 that represents the memory address in decimal notation. The number returned will also be a decimal integer with a range from 0 to 255.

In RetroBASIC, `PEEK` always returns zero.

#### Variations:

Digital Group MAXI-BASIC and North Star BASIC used `EXAM` for this purpose. This is not currently supported.

Digital Group Opus BASIC used `FETCH` for this purpose. Also not supported.
...
When POS is called with zero or one dummy parameter, it returns the current position of the cursor. This can be called after `PRINT` and `INPUT` statements to provide more control over output. When called with more than one parameter, it acts as an alias for `INSTR`, and is covered in the string functions section, above.

#### Variations:

Univac BASIC used `COL` for `POS`, which is not currently supported.

<!-- TOC --><a name="linaexp"></a>
### `LIN`(*aexp*)

`LIN` produces *aexp* newlines in the output. The exact nature of `LIN` varies across platforms. On HP systems it is a standalone statement, like `LIN 3`. Others treat it as a pseudo-function like `TAB` that is used within a `PRINT` statement. RetroBASIC treats it as a true function that returns a string, which can either be assigned to a variable or used directly within a `PRINT`.
...
`TAB` is used to line up output in tabular format, by specifying the same *aexp* for values that should be aligned. In most dialects, it can only be used in `PRINT` statements.

In most dialects, `TAB` is a pseudo-function that does not return a value; instead, it directly advances the cursor to the column given by *aexp*. For instance, in MS dialects, `A$=TAB(10)` will return a syntax error when run. In RetroBASIC, `TAB` *does* return a string, one containing enough spaces to advance from the current `POS` to the indicated column.

In Apple's Integer BASIC, `TAB` is a statement, not a function. To advance the cursor to column 10 and print "HELLO", one uses `TAB 10:PRINT "HELLO"`. Given this "function" does not return a value, this seems like a much better syntax. However, as the `TAB` is almost always used with a `PRINT`, the pseudo-functional style has the major advantage that you can place multiple `TAB`s in a single `PRINT` statement, whereas with IB's version these would have to be entirely separate statements.

For compatibility, RetroBASIC supports both the functional and statement-like variations.
...
The definition of a jiffy is platform-dependant. For instance, on Atari machines, it was 1/30th of a second for machines using NTSC, and 1/25th on those using PAL or SECAM. On the BBC Micro, it was 1/100th for all machines. This means that existing code that uses `TIME`, `TIME$` or `PAUSE` may work differently under RetroBASIC.
...
Most versions of BASIC include some form of file handling based on the `OPEN` and `CLOSE` statements. Unfortunately, that is where the commonality ends.

Devices on early computers varied widely in their underlying concepts as well as their implementations, such that sending data to a device like a tape drive might have entirely different syntax than sending the same data to a printer. Even those dialects that had the same underlying code, like Commodore BASIC and AppleSoft which were both based on the same original code from Microsoft, vary too widely in the file handling to allow code to be portable. Sometimes this is true even between models from the same company, like the PET vs. C64.
...
To ease the `ON ERR` trapping end-of-file errors, Business BASIC offered the `ON EOF#` statement, which watched for EOFs on the given channel and then performed the following statements. Unlike the `ON ERR` or even the normal `ON` statements, the item following `ON EOF#` was treated like an `IF`, not a branch. For instance, `ON EOF#5 PRINT"End of file in file 5"` will watch for the EOF error on channel 5 and then print a message, after which execution continues on the next statement in the program. To further confuse things, EOF trapping was turned off using the `OFF EOF#` statement. See `EOF` for RetroBASIC's solution to this problem.
...
`EOF` returns whether or not the channel in *aexp* has reached the end-of-file. This is normally used while reading data from an existing file. If the file is at the end, the function returns 1, otherwise 0.
...
According to Lien's *The BASIC Handbook*, "Some computers allow the word MAT to be optional", and suggests these allow `A=B` to perform what would normally be `MAT A=B`. However, no particular variant is mentioned, nor has one that allows this been found. Given most dialects treat A and A() as two different variables, it is not clear how they would understand the difference between normal statements and matrix statements. RetroBASIC does not allow the `MAT` to be left off.
...
*Illustrating BASIC* suggests you should *not* use `MAT INPUT`. This is because the command does not allow a prompt and there is no chance to print anything between each value being input. Worse, there is no way to test the results one-by-one, you will only know there is an issue after all of the values have been typed in. So if, for instance, you ask the user to type in values between 1 and 10 in a 5 by 5 matrix, and they type in 20 for the second item, there's no way to warn them until they've typed in the other 23 items and they will have to type in everything from the start. Using an `INPUT` in a `FOR` loop allows the values to be tested and re-typed individually.
...
4  
FILE NOT FOUND  
Attempt to open a file but the file cannot be found at the provided path, or the path is invalid.
