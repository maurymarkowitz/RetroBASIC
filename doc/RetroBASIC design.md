...
With the system up and running, it has been expanded with features from non-MS BASIC dialects. This allows it to run unmodified code from a variety of early BASICs that would otherwise require inter-dialect conversion.

## The three BASIC families

Today we think of "classic BASIC" as synonomous with MS BASIC, which was the standard on tens of millions of home computers. But as I read manuals from other platforms, three distinct families of BASIC became obvious and it turns out MS was a late-comer to the party. RetroBASIC's purpose to run early programs in MS-style expanded to include those variations wherever possible.
...
So along comes 1975 and the Altair 8800. Within a year there are dozens of microcomputer kits on the market, and every single one uses BASIC as a primary language. Dartmouth was a compiler that ran on mainframes, so that was going nowhere. That left you with many people who had worked with a BASIC interpreter on a DEC or HP machine.

When Gates, Allen and Davidoff began writing their BASIC for the Altair, they did so on the PDP-10, and natually the result looks a lot like DEC's BASIC-PLUS. In contrast, when Woz wrote Apple BASIC while working a day job at HP, it naturally ended up looking like HP BASIC. For a time, 1975 through about 1978, the two dialects were equally common and there are dozens of examples of both being used on various platforms.
...
5) IF..GOTO and IF..GOSUB found in some dialects
