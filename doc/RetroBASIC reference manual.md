...
A few dialects also added a *double* format as well, once again led by Microsoft's later versions like Microsoft Atari BASIC. These were indicated by adding the hash symbol, `#`, to the end of the variable name. In dialects that had doubles, it was common for the default internal format of the single to be smaller than normal and then have the programmer use doubles if more precision was required. For instance, on the Atari version of MS BASIC, a single was 32 bits instead of the more typical 40, while the double was 64 bits. On most of these dialects, a single-precision value could be explicitly indicated by adding the `!` suffix to the variable name, although this was the default format for variables, and the hash is almost never found in real-world programs.
...
Further confusion was added by the later addition of the `DEF` statements, which allow one or more variables to be given a type without using the suffix characters. For instance, one can `DEFSTR A` to indicate that A is a string variable without needing the \$. In this case, A and A$ will still be different variables, but both will hold strings.
...
Short for "dimension", `DIM` is used to set up arrays. At least one variable name and associated numeric expression has to be supplied. A variable with one numeric expression produces a one-dimensional array, sometimes referred to as a *vector*, while two-dimensional arrays are sometimes known as a *matrix*. In RetroBASIC, like most dialects, a single `DIM` statement can set up multiple arrays in a single statement.
...
The Color Genie required the address to be passed in hex format. This is supported for all numbers in RetroBASIC, and will work "naturally."
...
This program uses `DATA` to define the length and character data for two strings. It `READ`s the length and characters, and then loops to print the characters one at a time.
...
Wang BASIC interpreted the optional number parameter not as a line number but an ordinal, such that `RESTORE 10` would set the pointer to the 10th element in the program's `DATA`. As there is no way to tell which is which, RetroBASIC always interprets the value as a line number, which is far more common.
...
Because `RANDOMIZE` is so useful for debugging and many programs do not include this statement, RetroBASIC also allows the seed to be set on the command line using the `--random` switch. This allows you to set the seed without modifying the original program.
...
#### Examples:
    ...
    20 X$="SYSTEM"
    30 CHANGE X$ to X
    40 PRINT"The ASCII codes for the string `SYSTEM` are:"
    50 FOR I=1 TO X(0)
    60 PRINT X(I);",";
    70 NEXT I
    80 END
...
Most BASICs store their numbers internally using a binary format. This format cannot represent some decimal fractions exactly. For instance, the decimal number 0.1 when converted to binary becomes 0.00011001100110011... When multiple operations using these sorts of non-exact numbers are performed, the result may end up being rounded off, and no longer what is expected.

It is generally considered good form to only perform operations like `IF X=0...` if one is *sure* that the previous operations on X could not have resulted in this sort of rounding. For this reason, you may find code like `IF INT(X)=0...` or `IF X>=0...` in places where it would otherwise seem unexpected.

Tymshare SUPER BASIC took another approach, defining a pseudo-variable `EPS`, for *epsilon*, that represented the smallest possible value representable on that particular machine. One could then perform rounding-safe comparisons using the `=#`, the "close to equals" operator.
...
Rounds the number to the nearest integer or decimal place. If only one *aexp* is provided, that number is rounded to the "zeroth decimal", that is, a whole number. `ROUND(1.234)` will return 1. If two *aexp*s are provided, the first value is rounded to the second decimal place. So `ROUND(1.2345,3)` will return 1.235.
...
Sinclair QL BASIC uses `ATAN`, which is not used on any other of the Sinclair dialects.
...
Sinclair BASICs use `CODE` instead of `ASC`, which is further confused by the fact that the ZX machines do not use ASCII.
...
A number of BASICs, including AppleSoft, Atari, BBC, etc., used `GET` for this functionality, but most of these waited for a keystroke before continuing, and thus did not serve the same purpose. On these platforms, `INKEY$` was often implemented using `USR` or `PEEK`.
